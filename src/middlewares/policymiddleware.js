  ...
  User: {
    ...
    manage: (actor, target) => {
      if (sameId(actor._id, target._id))
        return "You cannot block or delete your own account";
      if (target.role === "admin")
        return "An admin account cannot be modified from here";
      return true;
    },
  },
  ...
  Order: {
    ...
    createReturn: (actor, order, req) => {
      ...
      if (item.status !== "delivered")
        return "Only a delivered item can be returned";
      if (item.returnRequested)
        return "A return has already been raised for this item";
...
const policy = (action, model) => (req, _res, next) => {
  ...
  if (!req.resource)
    throw apiError(
      500,
      `policy('${action}','${model}') ran without loadResource`,
    );
